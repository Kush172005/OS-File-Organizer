...
export default function RecycleBin({ onRestore, onPermanentDelete, onEmptyBin, onNavigate }) {
  ...
  if (trashFiles.length === 0 && !isRefreshing) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-slate-800 rounded-2xl border border-dashed border-slate-600">
        ...
        <h3 className="text-lg font-medium text-slate-200">Recycle Bin is empty</h3>
        <p className="text-sm text-slate-500 mt-1">Files you delete will appear here.</p>
        <button
            onClick={() => onNavigate("")}
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700/80 overflow-hidden">
      <div className="p-4 border-b border-slate-700 flex items-center justify-between bg-slate-800">
        <div>
          <h2 className="text-lg font-semibold text-slate-200">Recycle Bin</h2>
          <p className="text-xs text-slate-500 mt-0.5">{trashFiles.length} item{trashFiles.length !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={handleEmptyBin}
          className="px-3 py-1.5 text-xs font-medium bg-red-900/40 text-red-400 hover:bg-red-900/60 rounded-lg transition-colors flex items-center gap-1.5 border border-red-800/50"
        >
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-700/80 border-b border-slate-600">
              ...
              <th className="px-4 py-3 text-xs font-semibold text-slate-400 uppercase text-right">Size</th>
              <th className="px-4 py-3 text-xs font-semibold text-slate-400 uppercase">Deleted At</th>
              <th className="px-4 py-3 text-xs font-semibold text-slate-400 uppercase text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700/60">
            {trashFiles.map((file) => (
              <tr key={file.trashName} className="hover:bg-slate-700/40 transition-colors group">
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-slate-200">{file.originalName}</span>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className="px-2 py-0.5 rounded-full text-[10px] uppercase font-bold bg-indigo-500/20 text-indigo-400">
                    {file.category}
                  </span>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-400 text-right">
